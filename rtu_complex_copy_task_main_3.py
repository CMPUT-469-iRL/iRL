    ...
    for idx, batch in enumerate(train_data_loader):
        ...
        for src_token, tgt_token in zip(src, tgt): # for src_token, tgt_token in zip(src, tgt): #c=src y=tgt
            for sample in range(bsz): # loop through the batch to get each sample
                labels = tgt_token[sample].view(-1).to('cpu')
                ...
                src_token = torch.nn.functional.one_hot(src_token[sample], in_vocab_size)
                src_token = src_token.view(-1).squeeze(0).to('cpu', dtype = torch.float) #.squeeze(0)
