...
class FwdRealTimeNonLinearRTUs(nn.Module):
    ...
    def __call__(self,carry,x_t): 
        ...
        new_grad_memory_hc1_w_r = d_act_d_c1*(d_g_w_r * h_tminus1_c1 + g * grad_memory[0] - d_phi_w_r * h_tminus1_c2 - phi * grad_memory[1] + jnp.multiply(d_norm_w_r,w_c1_x_t))
        new_grad_memory_hc2_w_r = d_act_d_c2*(d_g_w_r * h_tminus1_c2 + g * grad_memory[1] + d_phi_w_r * h_tminus1_c1 + phi * grad_memory[0] + jnp.multiply(d_norm_w_r,w_c2_x_t))
        
        new_grad_memory_hc1_w_theta = d_act_d_c1*(d_g_w_theta * h_tminus1_c1 + g * grad_memory[2] - d_phi_w_theta * h_tminus1_c2 - phi * grad_memory[3])
        new_grad_memory_hc2_w_theta = d_act_d_c2*(d_g_w_theta * h_tminus1_c2 + g * grad_memory[3] + d_phi_w_theta * h_tminus1_c1 + phi * grad_memory[2])
       
         
        new_grad_c1_wx1 = jnp.expand_dims(d_act_d_c1,1)*(jnp.multiply(g,grad_memory[4]) - jnp.multiply(phi,grad_memory[5]) + jnp.multiply(norm,jnp.repeat(jnp.expand_dims(x_t,2),h_t_c1.shape[-1],axis=2)))
        new_grad_c1_wx2 = jnp.expand_dims(d_act_d_c1,1)*(jnp.multiply(g,grad_memory[6]) - jnp.multiply(phi,grad_memory[7]) )
        
        new_grad_c2_wx1 = jnp.expand_dims(d_act_d_c2,1)*(jnp.multiply(g,grad_memory[5]) + jnp.multiply(phi,grad_memory[4]) )
        new_grad_c2_wx2 = jnp.expand_dims(d_act_d_c2,1)*(jnp.multiply(g,grad_memory[7]) + jnp.multiply(phi,grad_memory[6]) + jnp.multiply(norm,jnp.repeat(jnp.expand_dims(x_t,2),h_t_c2.shape[-1],axis=2)))
        ...
        return ((h_t_c1_post_act,h_t_c2_post_act),jax.lax.stop_gradient(new_grad_memory)),((h_t_c1_post_act,h_t_c2_post_act),jax.lax.stop_gradient(new_grad_memory))
